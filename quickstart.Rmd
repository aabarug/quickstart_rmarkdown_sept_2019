...
author: "Adele Barugahare<br>@technadele<br>https://bit.ly/2lyGAqr"
date: "27/08/2019"
output: 
...
```{r}
rmarkdown::paged_table(head(df, 50))
```
...
```{r, ggplotly_code_block, echo=T, eval=TRUE}
plotly::ggplotly(gp)
```
...
---
```{r, shiny_app_launch, eval=FALSE, echo=T}
# Run the application 
shinyApp(ui = ui, server = server)
```
...
Different documents have different options to customise e.g PDF documents can embed LaTeX and will have YAML options specific to LaTeX that cannot be used in html_documents
